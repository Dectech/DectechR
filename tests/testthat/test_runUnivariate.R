...
test_that("runUnivariate() on a simple linear regression", {
    ...
    expect_equal(u1$`p-value`, c(5.400948e-03, 1.084446e-03, 1.787835e-07))
    ...
    expect_equal(u2$`p-value`, c(5.400948e-03, 1.084446e-03, 1.787835e-07))
...
test_that("runUnivariate() on a lm() with factor var", {
    ...
    expect_equal(u1$`p-value`, c(0.00540094822470767, 0.234346327455179, 0.0128499280141855, 0.000529547727433719, 0.291736245223966, 0.0592726905726684, 1.78783525412108e-07))
    expect_identical(u1$`(Intercept)`, NULL)
    ...
    expect_equal(u2$`p-value`, c(0.00540094822470767, 0.234346327455179, 0.0128499280141855, 0.000529547727433719, 0.291736245223966, 0.0592726905726684, 1.78783525412108e-07))
    expect_identical(u2$`(Intercept)`, c(5.62333333333333, 25.3428571428571, NA, NA, NA, NA, 30.0988605396225))
...
test_that("runUnivariate() on a simple binary logit regression, glm()", {
    ...
    expect_equal(u1$`p-value`, c(0.0422395979083261, 0.00640050430527483, 0.0055543185620774))
    expect_identical(u1$`(Intercept)`, NULL)
    ...
    expect_equal(u2$`p-value`, c(0.0422395979083261, 0.00640050430527483, 0.0055543185620774))
...
test_that("runUnivariate() on a glm() with factor var", {
    ...
    expect_equal(u1$`p-value`, c(0.0422395979083261, 0.270585575135868, 0.995686729653739, 0.0175830266863531, 0.997509724050083, 0.997509724050099, 0.0055543185620774))
    expect_identical(u1$`(Intercept)`, NULL)
    ...
    expect_equal(u2$`p-value`, c(0.0422395979083261, 0.270585575135868, 0.995686729653739, 0.0175830266863531, 0.997509724050083, 0.997509724050099, 0.0055543185620774))
    expect_equal(u2$`(Intercept)`, c(-4.48716959736612, 1.79175946922806, NA, NA, NA, NA, 5.74156215209908))
...
test_that("runUnivariate() on a polr() with factor var", {
    ...
    ## writeClipboard(paste0(u1$`p-value`, collapse = ", "))
    ...
    expect_equal(u2$`[-10.8,-1.35]|(-1.35,1.39]`, c(-1.62346998988311, -0.703315115838237, -1.28476323803524, NA))
    expect_equal(u2$`(-1.35,1.39]|(1.39,10.2]`, c(1.71449585518792, 0.728721708424453, 0.170377819294584, NA
))
})
