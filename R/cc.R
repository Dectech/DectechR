...
cc2 = function(data, destination = NA, includeRowNames = FALSE){
    ...
    #--(1) If file name given, write directly to file, otherwise will write to clipboard...
    if (!is.na(destination)){

        # if file name given then write to file...
        write.table(data,destination,sep="\t",row.names=F)
        ...
        if (isNestedTable){
            ...
            data=reshape(data,v.names=myValues,timevar = myColumnVar, idvar=myOtherVars,direction="wide")
